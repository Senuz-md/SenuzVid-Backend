...
const app = express();
app.use(cors()); // CORS must be immediately after express init
...
app.get("/api/details", async (req, res) => {
  ...
  try {
    ...
    return res
      .status(500)
      .json({ error: `${platform} fetch failed`, details: err.message });
  }
});
...
app.get("/api/download", async (req, res) => {
  ...
  try {
    ...
    return res
      .status(500)
      .json({ error: `${platform} download failed`, details: err.message });
  }
});
